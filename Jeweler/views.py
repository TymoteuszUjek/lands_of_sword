...
def jeweler(request):
    ...
    items_for_sale = Item.objects.filter(shop_type=' Jeweler').exclude(
    ...
    if request.method == 'POST':
        ...
        if action == 'buy':
            ...
            # Pobierz aktualny czas po wykonaniu akcji
            current_time = timezone.now()

            # Zaktualizuj licznik tylko w przypadku zakupu
            transaction = Transaction(user=request.user, item=item, transaction_type='buy', price=item.price, transaction_date=current_time)
            transaction.save()
            
            return HttpResponseRedirect(reverse('Jeweler:jeweler'))
            ...
            if inventory_items.exists():
                ...
                # Nie zaktualizuj licznika w przypadku sprzedaży
    ...
    return render(request, 'Jeweler/jeweler.html', context)